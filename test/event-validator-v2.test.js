...
const template = TEMPLATES["boda-civil-esencial"];
const originalSections = template.supportedSections;
const originalModules = template.supportedModules;
try {
    template.supportedSections = originalSections.filter((type) => type !== "hero");
    expectError(validEvent(), "type no soportado");
    template.supportedSections = originalSections;

    template.supportedModules = originalModules.filter((slug) => slug !== "music");
    expectError(validEvent({ modules: { music: { enabled: true } } }), "modules.music no esta soportado");
} finally {
    template.supportedSections = originalSections;
    template.supportedModules = originalModules;
}
...
assert.strictEqual(validateV2Event(validEvent({
    schedule: { date: "2027-04-17" },
    sections: [{ id: "countdown", type: "countdown", enabled: true, order: 10, data: {} }]
})).valid, true);
assert.strictEqual(validateV2Event(validEvent({
